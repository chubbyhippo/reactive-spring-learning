...
public class SimpleFluxFactoriesTest {
	...
	public void simple() {
		...
		StepVerifier.create(greetingMono).expectNext(new Date(now))
				.verifyComplete();

		Mono<Object> empty = Mono.empty();
		StepVerifier.create(empty).verifyComplete();

		Flux<Integer> fromArray = Flux.fromArray(new Integer[]{
				1, 2, 3
		});
		...
		StepVerifier.create(integerFlux.take(3)).expectNext(1).expectNext(2)
